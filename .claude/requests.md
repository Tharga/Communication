...
- **Status:** Done (2026-04-01) — Type-based and data-based subscriptions via SubscribeAsync<T>(key?). Client-side HasSubscribers<T>(key?) and PostIfSubscribedAsync<T>. Server-side GetSubscriptions() for monitoring. Notification sent to Tharga.MongoDB.
